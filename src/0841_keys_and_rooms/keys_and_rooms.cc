...
// descriptions: https://leetcode-cn.com/problems/keys-and-rooms
#include <bits/stdc++.h>
#include <iostream>
#include <numeric>
#include <vector>
...
private:
    vector<bool> flags;
    void traverse(vector<vector<int>>& rooms, int roomId)
    {
        // 如果该房间已经访问过
        if(flags[roomId]) return;

        flags[roomId] = true;
        for(int i = 0; i < rooms[roomId].size(); i++)
        {
            traverse(rooms, rooms[roomId][i]);
        }
    }
public:
    // 1.并查集
    ...
    {
        flags = vector<bool>(rooms.size(), false);
        traverse(rooms, 0);
        auto pos = std::find(flags.begin(), flags.end(), false);
        if(pos == flags.end()){
            return true;
        }
        return false;
